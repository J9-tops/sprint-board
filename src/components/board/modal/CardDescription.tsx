...
import { AlignLeft, Bold, Italic, List, ListOrdered, Underline as UnderlineIcon } from "lucide-react";
    ...
    <div className="flex items-start gap-4">
      ...
      <div className="flex-1 space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="font-bold text-base tracking-tight text-foreground">Description</h3>
          <Button variant="secondary" size="sm" className="h-8 font-bold text-xs uppercase tracking-widest px-3 focus-visible:ring-2 focus-visible:ring-primary outline-none border border-transparent">
            Edit
          </Button>
        </div>
          ...
          <div className="flex items-center gap-1 p-1 mb-1 bg-muted/50 rounded-md border border-border/50 opacity-0 group-focus-within:opacity-100 transition-opacity">
            {toolbarButtons.map((btn, i) => (
              <Button
                ...
                variant="ghost"
                size="icon"
                className={cn("h-8 w-8", editor.isActive(btn.active) && "bg-muted text-primary")}
                onClick={btn.action}
              >
                <btn.icon size={14} />
              </Button>
