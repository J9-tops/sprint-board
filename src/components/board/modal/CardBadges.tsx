    ...
    <div className="flex flex-wrap gap-x-12 gap-y-6 px-10">
      {labels && labels.length > 0 && (
        <div className="space-y-2">
          <h4 className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest px-0.5">
            Labels
          </h4>
          <div className="flex flex-wrap gap-2">
            {labels.map((label, i) => (
              <button
                key={i}
                className={cn(
                  'h-8 px-3 text-xs font-bold border-none text-white rounded-md focus-visible:ring-2 focus-visible:ring-primary outline-none hover:opacity-90 transition-opacity flex items-center',
                  label.color,
                )}
              >
      ...
      {dueDate && (
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2 h-8 px-3 rounded-md bg-muted hover:bg-muted/80 cursor-pointer transition-colors group focus-within:ring-2 focus-within:ring-primary outline-none">
            ...
            <Badge
              variant="secondary"
              className="h-4 px-1 text-[9px] font-black bg-red-500/10 text-red-500 border-none uppercase tracking-wider"
            >
              Overdue
            </Badge>
          </div>
